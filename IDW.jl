    ...
    function computedualIDW(x_train::Vector{Dict}, y_train::Vector{Float64}, x::Dict, dual_dic::Dict)
        w = zeros(length(x_train))
        u_up = 0.0
        u_down = 0.0
        p = 40
        ...
        for i in 1:length(x_train)
            ...
            for (key,value) in x
                ...
                if haskey(dual_dic, temp_sym)
                    d += (value - x_train[i][key])^2*abs(dual_dic[temp_sym])
                end
            end
            d = sqrt(d)
            w[i] = 1 / (d^p)
