...
name_str =string("scr",scr_int, "_", res_int,"h_v00", surroSelect_sym,"_gap",gap)
...
for (id,s) in enumerate((benders_obj.sub)) 
    subData[s] = SubObj() 
    worker_status[id+1] = true
end
time_df = DataFrame(i= Int[], worker_id = [], type = [], time = [])
        ...
        while !isempty(job_queue)
            ...
            for (worker_id, is_free) in worker_status
                if is_free && !isempty(job_queue)
                    ...
                    append!(time_df, DataFrame(i= benders_obj.itr.cnt.i, worker_id = worker_id, type = :str, time = now()))
                    popfirst!(job_queue)
            ...
            for worker_id in keys(status.futworkers_dic)
                if isready(status.futworkers_dic[worker_id])
                    ...
                    append!(time_df, DataFrame(i= benders_obj.itr.cnt.i, worker_id = worker_id, type = :res, time = now()))
...
CSV.write(benders_obj.report.mod.options.outDir * "/trackingTime_$(benders_obj.info.name).csv", time_df)
