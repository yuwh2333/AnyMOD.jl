...
while true
	...
	for (id,s) in enumerate(collect(keys(benders_obj.sub)))
		if benders_obj.algOpt.dist # distributed case
            ...
            if s in cut_group
                ...
                actCost_temp = track_itr[length(track_itr)][id, :actCost]
            end
	...
	for i in 1:nrow(track_itr_temp)
		if i == argmax(track_itr_temp[!,:diff]) 
            ...
            push!(cut_group, sLargeDif_tup)			
		else 
			track_itr_temp[i,:largest] = 0
            if track_itr_temp[i, :diff] <= -1
                push!(cut_group, track_itr_temp[i, :s])
            end
		end
	end
	...
	#filter!(x -> x[1] == sLargeDif_tup || isless(track_itr_temp[[s == x[1] for s in track_itr_temp.s], :diff][1],-1) , benders_obj.cuts)
...
CSV.write("track_df_trys6.csv",track_df)
