...
while true
    ...
    if surroSelect_sym == :dist
        ...
        status.check_Conv = length(inputvr)>1 && (L2NormDict(input,inputvr[length(inputvr)]) < sqrt(10*length(input)) || benders_obj.itr.gap < gap || status.last_stab_obj == benders_obj.stab.objVal) ? true : false
    end
    push!(inputvr,input) 
